...
module.exports = function (configuration) {
	...
	return {
		_service: docClient,
		batchGetHashkey: (table, hashkey, ids, opts = {}) => {
						...
						tasks.push(function (done) {
							...
							self.batchTableWrite(table, myRecords).then(unprocessedItems => {
								if (unprocessedItems && Object.keys(unprocessedItems).length) {
									myRecords = self.data.UnprocessedItems[table];
									retry.backoff();
								} else {
		 ...
		 */
		writeToTableInChunks: (table, opts) => {
			opts = merge({
				chunk_size: 25,
				concurrency: 10,
